...
dest="$HOME/OpenROAD-flow-scripts/flow/designs/nangate45/SimpleCPU"
flow_dir="$HOME/OpenROAD-flow-scripts/flow"
report_dir="$flow_dir/reports/nangate45/SimpleCPU/base"
...
for folder in "${folder_list[@]}"; do
    ...
    folder_path="$HOME/Assignments3/$folder/Part3"
    memory_path="$HOME/Assignments3/memory"   
...
with open("synth_stat.txt") as f:
    for line in f:
        if "Chip area for" in line:
            # 匹配 "Chip area for top module '\SimpleCPU': ..." 或 "Chip area for module 'SimpleCPU': ..." 等格式
            m = re.search(r"Chip area for (?:top )?module '\\?SimpleCPU':\s*([\d\.]+)", line)
            if m:
                chip_area = m.group(1)
    if chip_area:
        print(chip_area)
