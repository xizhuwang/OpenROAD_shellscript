...
dest="$HOME/OpenROAD-flow-scripts/flow/designs/nangate45/CompMultiplier"
flow_dir="$HOME/OpenROAD-flow-scripts/flow"
report_dir="$flow_dir/reports/nangate45/CompMultiplier/base"
...
for folder in "${folder_list[@]}"; do
    ...
    # 資料夾位於 ~/Assignments 下
    folder_path="$HOME/Assignments2/$folder/Part1"
    ...
    cp "$folder_path/Product.v" "$dest/"
    cp "$folder_path/CompMultiplier.v" "$dest/"
    cp "$folder_path/Multiplicand.v" "$dest/"
...
with open("synth_stat.txt") as f:
    for line in f:
        if "Chip area for" in line:
            # 匹配 "Chip area for top module '\CompMultiplier': ..." 或 "Chip area for module 'CompMultiplier': ..." 等格式
            m = re.search(r"Chip area for (?:top )?module '\\?CompMultiplier':\s*([\d\.]+)", line)
            if m:
                chip_area = m.group(1)
    if chip_area:
        print(chip_area)
...
EOF
    ...
    # 7. 刪除目的資料夾內先前複製的檔案
    rm -f "$dest/ALU.v" "$dest/Control.v" "$dest/Product.v" "$dest/CompMultiplier.v" "$dest/Multiplicand.v" 
done
