...
dest="$HOME/OpenROAD-flow-scripts/flow/designs/nangate45/CompALU"
flow_dir="$HOME/OpenROAD-flow-scripts/flow"
report_dir="$flow_dir/reports/nangate45/CompALU/base"
...
for folder in "${folder_list[@]}"; do
    ...
    # 資料夾位於 ~/Assignments 下
    folder_path="$HOME/Assignments/$folder"
    ...
    # 1. 複製檔案到目的資料夾
    cp "$folder_path/CompALU.v" "$dest/"
    cp "$folder_path/ALU.v" "$dest/"
    cp "$folder_path/RF.v" "$dest/"
...
with open("synth_stat.txt") as f:
    for line in f:
        if "Chip area for" in line and "CompALU" in line:
            # 匹配 "Chip area for module 'CompALU': ..." 與 "Chip area for top module '\CompALU': ..." 兩種格式
            m = re.search(r"Chip area for (?:top )?module '\\?CompALU':\s*([\d\.]+)", line)
            if m:
                chip_area = m.group(1)
                break
if chip_area:
...
EOF
    ...
    # 7. 刪除目的資料夾內先前複製的檔案
    rm -f "$dest/CompALU.v" "$dest/ALU.v" "$dest/RF.v"
done
